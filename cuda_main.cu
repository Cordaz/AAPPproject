...
__device__ void gpu_strncpy(char * a, char* b, unsigned short int l){
    unsigned short int i=0; //TODO CECI!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    do{
        a[i] = b[i];
        i++;
    }
...
__global__ void voting(char ** reads, unsigned short int *** voting_matrix_array, unsigned int inputDim, uint64_t * gpu_hashed_spectrum, unsigned int spectrum_size){ //TODO
	...
	for(h=0; h<DATA_PER_THREAD; h++){
	     ...
	     *
	     */
        voting_matrix = (unsigned short int **)voting_matrix_array[inputDim/DATA_PER_THREAD*h + idx];
        read = (char *)reads[inputDim/DATA_PER_THREAD*h + idx];
				    ...
				    for(c=0; c<BASES; c++){
					    if(subseq[p] != bases[c]){
							...
							if(CheckHash(gpu_hashed_spectrum, subcpy, spectrum_size))
							    voting_matrix[j+p][c]++;  //increment number of solid tuples with that correction
					    }
				    }
			    }
		    }
